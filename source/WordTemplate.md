...
- `width` 和 `height` 是可选参数，用于指定二维码的宽度和高度。如果不指定，二维码将按照5cm x 5cm的大小显示。如果需要指定，需要使用**厘米**作为单位。例如 `{@二维码 | _qrcode:10:10}` 表示将二维码插入到文本中，宽度为10cm，高度为10cm。
- 二维码占位符必须以`@`开头，例如 `{@二维码 | _qrcode}` 中的 `@` 就是二维码占位符的开始符号。
- 二维码占位符必须包含二维码类型（`| _qrcode`），否则工具无法识别, 其中`|`, `_`需要使用**英文字符**
...
![鼠标二维码](_static/qrcodemouse.png)

键盘

![键盘二维码](_static/qrcodekeyboard.png)
...
如需要将 Excel 数据写入 Word 模板，并显示为 Word 表格，可以使用表格占位符。表格占位符不是独立的占位符，它是循环占位符和判断占位符的组合，所以请务必先了解循环占位符和判断占位符。

#### 表格占位符1

表格占位符1适用于 Word 模板中只有一个表格，这个表格只有一个表头，希望将 Excel 数据写入表格数据中。最终在 Word 中形成一个表格。
...
表格占位符2适用于 Word 模板中只有一个表格，但是希望 Excel 的每行的数据都写入一个 Word 表格模板，最终在 Word 文档中出现若干个表格 。
...
表格占位符适合于较为复杂的表格场景。可以通过如下的例子来辅助理解表格占位符 3.

表格占位符 3 中所使用的数据源不再是 Excel 格式的数据了，而是 JSON 格式。如果你希望将 Excel 转换成 JSON，可以使用Excel插件 [Excel-to-JSON-by-WTSolutions](https://excel-to-json.wtsolutions.cn/zh-cn/latest/ExcelAddIn.html)
